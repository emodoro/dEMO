#' @title ROCs AREAS por p values interval [0,0.05]
#' @description ROCAreas005 is a functions which allow plotting Area under ROCs for interval of p values [0,0.05] and for several hypothesis testing methods, shuch a several Differential Expression analysis.
...
#' @return a box plot with the AREA under ROC, for p values interval equal to [0,0.05], for every and each one of hypothesis test methods
#' @export ROCAreas005
#' @examples
#' library(compcodeR)
#' library(edgeR)
...
ROCAreas005<-function(dataRoc, resample=10,main, DE=0.1){
  require(pracma)
  ...
  for(j in 1:resample){
    ...
    for(k in 1:(dim(dataRoc)[2]-1)){
      areas005[,k]<-trapz(y = TPr[FPr[,k]<=0.05,k],x=FPr[FPr[,k]<=0.05,k])
  ...
  par(mar=c(8,7,5,0.5))
  boxplot(AREAS005, main=paste("ROC area 0.05 \n", main), ylim=c(0,0.05), cex.axis=2, las=2, cex.lab=2, ylab="Areas\n ")
}
